...
It makes it so that the part thats indented under it will only run when you're running the program itself as a script, and will not run when you import the same codde to be used as a module in another program. 
...
In the end, its an unwritten code to be followed by python devs to differentiate between a module that needs to be imported(do not include and a script that needs to be run.
